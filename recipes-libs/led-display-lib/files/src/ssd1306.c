...
uint8_t ssd1306_oled_load_resolution()
{
    ...
    // file exists
    if (fscanf(fp, "%hhux%hhu", &max_columns, &max_lines) == 2)
    {
        printf("ssd1306_oled_load_resolution loaded: %hhu x %hhu\n",
                max_columns, max_lines);
    }
    else
    {
        printf("ssd1306_oled_load_resolution failed!\n");
    }
